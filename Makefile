# This Makefile is for the Vigil::Token extension to perl.
#
...
#     ABSTRACT => q[Cryptographically secure, URL-safe token generator for OTPs, sessions, and custom codes.]
...
#     NAME => q[Vigil::Token]
#     PREREQ_PM => { Bytes::Random::Secure=>q[0], MIME::Base64=>q[0], Math::BigInt=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Vigil/Token.pm]
...
PARENT_NAME = Vigil
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Vigil/Token.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
DISTNAME = Vigil-Token
DISTVNAME = Vigil-Token-2.0.0
...
Token.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Vigil::Token\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "abstract: 'Cryptographically secure, URL-safe token generator for OTPs, sessions, and custom codes.'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "name: Vigil-Token" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "   \"abstract\" : \"Cryptographically secure, URL-safe token generator for OTPs, sessions, and custom codes.\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   }," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"Vigil-Token\"," >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) "<SOFTPKG NAME=\"Vigil-Token\" VERSION=\"2.0.0\">" > Vigil-Token.ppd
	$(NOECHO) $(ECHO) "    <ABSTRACT>Cryptographically secure, URL-safe token generator for OTPs, sessions, and custom codes.</ABSTRACT>" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Jim Melanson &lt;jmelanson1965@gmail.com&gt;</AUTHOR>" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "    <IMPLEMENTATION>" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "        <PERLCORE VERSION=\"5,010000,0,0\" />" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Bytes::Random::Secure\" />" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"MIME::Base64\" />" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Math::BigInt\" />" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "        <ARCHITECTURE NAME=\"MSWin32-x64-multi-thread-5.32\" />" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "        <CODEBASE HREF=\"\" />" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) "    </IMPLEMENTATION>" >> Vigil-Token.ppd
	$(NOECHO) $(ECHO) ^</SOFTPKG^> >> Vigil-Token.ppd
...
pm_to_blib({@ARGV}, '$(INST_LIB)\auto', $$filter, '$(PERM_DIR)')" -- $(PM_FILTER) -- \
	  lib/Vigil/Token.pm blib\lib\Vigil\Token.pm 
